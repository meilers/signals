...
$sql = "SELECT me.cityId, IF((me.cityId = u.cityId),0,1) AS a,
		...
		IF((u.userId IN (SELECT userId FROM users_invitation_requests WHERE candidateId=$userId AND requestTime >= DATE_SUB(NOW(),INTERVAL 24 HOUR))),1,0) AS invitationReceived
...
if (mysql_num_rows($query) > 0)
{
	...
	{
		$sql = "INSERT INTO users_invitation_requests (userId, candidateId, cityId, requestTime) VALUES ($userId, $personId, $cityId, CURRENT_TIMESTAMP())";
		$query = mysql_query($sql);

		if ($query)
		{
			$potentialInvitation = NULL;
			...
			if( $row['invitationReceived'] == 1 )
			{
				...
				$sql = "INSERT INTO users_potential_invitations (userId, candidateId, placeId0, placeId1, placeId2, placeId3, placeId4, placeId5, placeId6, placeId7, placeId8, placeId9, createTime) 
				...
				$potentialInvitation=array(
					'places' => $places
		        );
		
				echo json_encode(array( 'ok' => '1', 'r' => $potentialInvitation ));
			}
			else
