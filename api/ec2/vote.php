...
$sql = "SELECT IF((me.placeId = u.placeId AND me.placeId != 0),0,1) AS a, IF(me.voteId = 0,0,1) AS b,
		IF((u.userId) NOT IN
...
$query = mysql_query($sql);
	...
	if (mysql_num_rows($query) > 0)
	{
		...
		else
		{
		    ...
		    $result1 = mysql_query("UPDATE users SET voteId = $personId, voteTime = CURRENT_TIMESTAMP() WHERE userId = $userId");
		    $result2 = mysql_query("INSERT INTO users_votes (userId, userVotedId, placeId, voteTime) VALUES ($userId, $personId, $placeId, CURRENT_TIMESTAMP())");
			...
			if( $result1 && $result2 )
			{
			    ...
			    if (mysql_num_rows($query) > 0)
			    {
			        ...
			        $data = array(
			            ...
			            'type' => 'android',
			            'data' => array(
			                'alert' => $row['username'] . ' voted for you!',
			                'title' => '@ ' . $row['name'],
