...
if( $radius == 1 )
    ...
    $sql = "SELECT userId, username, placeId, name, genre, address, latitude, longitude, COUNT(userId) FROM
			...
			FROM users WHERE userId = $userId) AS me, places as p, users_votes as uvo, cities as c
            WHERE p.cityId = c.cityId AND c.city = '$city' AND c.state = '$state' AND c.country = '$country' 
			AND u.placeId != 0 AND u.placeId = p.placeId AND u.placeId = uvo.placeId AND uvo.userVotedId = u.userId AND date(now() ) = date(uvo.voteTime) AND hour(now() ) = hour(uvo.voteTime)
			...
			SELECT u.userId, u.username, p.placeId, p.name, p.genre, p.address, p.latitude, p.longitude FROM users AS u, (SELECT sex, interestedIn, wantedMinAge, wantedMaxAge, birthdate 
			FROM users WHERE userId = $userId) AS me, places as p, cities as c
	        WHERE p.cityId = c.cityId AND c.city = '$city' AND c.state = '$state' AND c.country = '$country'
            AND u.placeId != 0 AND u.placeId = p.placeId
